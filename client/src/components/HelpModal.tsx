  ...
  return (
    <div className="help-content">
      ...
      <ul>
        ...
        <li><strong>Interactive checklist</strong> — <code>```checklist</code> takes <code>{`{ title?, description?, items: [{ text, done? }] }`}</code>. Renders a clickable todo/progress card with a percentage bar; <em>ticking a box writes back to the markdown source immediately</em> (page version bumped, revision snapshot, FTS reindexed). AI can do the same via the <code>toggle_checklist_item</code> MCP tool — e.g. "tick @47 item 1"</li>
  ...
  return (
    <div className="help-content">
      ...
      <ul>
        ...
        <li><strong>Checklist (interactive)</strong> — <code>```checklist</code> รับ <code>{`{ title?, description?, items: [{ text, done? }] }`}</code>. Render เป็นการ์ด todo มี progress bar; <em>คลิกถูก/ผิดที่ checkbox จะเขียนกลับ markdown ทันที</em> (bump page version + revision snapshot + FTS reindex). AI ใช้ MCP tool <code>toggle_checklist_item</code> ก็ได้ผลเดียวกัน (เช่น "tick @47 item 1")</li>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>toggle_checklist_item</code></td><td>Flip the <code>done</code> state of one item in a <code>```checklist</code> fence. Args: <code>{`{ block_id, index, done }`}</code>. Locates the block by its <code>@N</code> id, mutates the JSON, and writes it back to the page source — same path the web UI uses when a user clicks a checkbox</td></tr>
        </tbody>
      </table>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>toggle_checklist_item</code></td><td>กลับสถานะ <code>done</code> ของ item ใน <code>```checklist</code> fence. Args: <code>{`{ block_id, index, done }`}</code>. ค้นหา block ตาม <code>@N</code>, แก้ JSON, แล้ว write กลับ markdown source — เส้นทางเดียวกับที่ web UI ใช้ตอน user ติ๊ก checkbox</td></tr>
        </tbody>
      </table>
