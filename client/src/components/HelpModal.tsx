    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>move_page</code></td><td>Move a page so it sits immediately <strong>before / after</strong> another page in the same knowledge — pass exactly one of <code>before</code> / <code>after</code>. Cheaper than <code>reorder_pages</code> when nudging a single page</td></tr>
          <tr><td><code>move_page_to</code></td><td>Move a page to an absolute 1-based slot — <code>position: 1</code> makes it first, <code>position: N</code> (page count) makes it last; other pages shift around</td></tr>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>get_table_rows</code></td><td>Get a contiguous range of rows — <code>start</code> plus either <code>end</code> (inclusive) or <code>offset</code> (count); 0-based, negative wraps from the end. Range is clamped and trimmed by <code>limit</code> (default 100 / max 500). Sliding-window reads over large tables instead of pulling the whole body</td></tr>
          ...
          <tr><td><code>append_table_rows</code></td><td>Append raw <code>| a | b |</code> rows after the last data row — the trailing <code>{`{@N}`}</code> annotation stays put. <code>expected_version</code> gates stale writes; returns the new row indices. (<code>append_table_row</code> = compatibility alias)</td></tr>
          <tr><td><code>insert_table_rows</code></td><td>Insert rows <strong>before</strong> 0-based row <code>at</code> — <code>at: 0</code> prepends, <code>at = row_count</code> appends. Rows are validated up-front, no half-writes. (<code>insert_table_row</code> = compatibility alias)</td></tr>
          <tr><td><code>update_table_rows</code></td><td>Replace a contiguous row range with <code>new_rows</code> — the table may shrink or grow. Header, separator + trailing <code>{`{@N}`}</code> line are preserved; <code>expected_version</code> recommended</td></tr>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>move_page</code></td><td>ย้าย page ให้ไปอยู่<strong>ก่อน / หลัง</strong> page อื่นใน knowledge เดียวกัน — ส่งอย่างใดอย่างหนึ่งระหว่าง <code>before</code> / <code>after</code>. ถูกกว่า <code>reorder_pages</code> เมื่อขยับแค่ page เดียว</td></tr>
          <tr><td><code>move_page_to</code></td><td>ย้าย page ไป slot ที่ระบุ (1-based) — <code>position: 1</code> = แรกสุด, <code>position: N</code> (จำนวน page) = ท้ายสุด; page อื่นเลื่อนให้เอง</td></tr>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>get_table_rows</code></td><td>ดึงช่วงแถวต่อเนื่อง — <code>start</code> + อย่างใดอย่างหนึ่งระหว่าง <code>end</code> (inclusive) หรือ <code>offset</code> (จำนวน); 0-based, เลขลบนับจากท้าย. ช่วงถูก clamp + ตัดด้วย <code>limit</code> (default 100 / max 500). ใช้อ่านตารางใหญ่แบบ sliding window แทนดูดทั้ง body</td></tr>
          ...
          <tr><td><code>append_table_rows</code></td><td>เพิ่มแถว <code>| a | b |</code> ต่อท้ายแถวข้อมูลสุดท้าย — บรรทัด annotation <code>{`{@N}`}</code> อยู่ที่เดิม. <code>expected_version</code> ไว้ gate stale write; คืน index ของแถวใหม่. (<code>append_table_row</code> = alias เดิม)</td></tr>
          <tr><td><code>insert_table_rows</code></td><td>แทรกแถว<strong>ก่อน</strong>แถวที่ <code>at</code> (0-based) — <code>at: 0</code> = บนสุด, <code>at = row_count</code> = ต่อท้าย. validate ทุกแถวก่อนเขียน ไม่มี half-write. (<code>insert_table_row</code> = alias เดิม)</td></tr>
          <tr><td><code>update_table_rows</code></td><td>แทนที่ช่วงแถวด้วย <code>new_rows</code> — ตารางหดหรือขยายได้. header, separator + บรรทัด <code>{`{@N}`}</code> ท้ายตารางคงเดิม; แนะนำส่ง <code>expected_version</code></td></tr>
          ...
          <tr><td><code>reveal_secret</code></td><td>ถอดรหัส 1 block — <code>{`{ block_id }`}</code> (<code>@N</code>) หรือ <code>{`{ page_id, label? | index? }`}</code> + <code>key?</code> (ไม่ส่ง = ลอง key ของ server). คืน <code>{`{ text, label, hint, block_id, page_id, index, key_source, url }`}</code>. key ผิด → error; หลาย block แต่ไม่ระบุ → error พร้อมรายชื่อ. เคารพสิทธิ์ view ของ project; ทุกครั้งที่ถอดจะลง activity log (ไม่บันทึกข้อความ)</td></tr>
