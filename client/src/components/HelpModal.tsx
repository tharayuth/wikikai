  ...
  return (
    <div className="help-content">
      ...
      <ul>
        ...
        <li><strong>แนบไฟล์ — <code>```file</code> fence</strong> — ไฟล์อะไรก็ได้ที่ให้ดาวน์โหลด (PDF, CSV, XLSX, ZIP, …). AI อัปโหลดด้วย <code>add_file</code> แล้ววาง fence ที่ได้กลับมา: JSON <code>{`{ src, name, size_bytes, mime, description? }`}</code> (หรือ array ถ้าหลายไฟล์). แสดงเป็นการ์ดพร้อมชื่อไฟล์ ขนาด ชนิด คำอธิบาย ปุ่ม <strong>View</strong> (เปิดดูใน tab ใหม่ได้กับ PDF, รูป, text/CSV/JSON, เสียง, วิดีโอ — ชนิดอื่นมีแค่ Download เพื่อไม่ให้ไฟล์ที่อัปโหลดรันเป็นหน้าเว็บในระบบ) และปุ่ม <strong>Download</strong>; ไฟล์ที่โหลดออกไปได้ชื่อ <em>เดิม</em> แม้ server จะเก็บด้วยชื่อ hash ที่เดาไม่ได้. เมื่อทุกหน้าที่อ้างถึงไฟล์ตัดมันออก — AI แก้, ลบ page / ลบ topic, หรือคนกด <Kbd>Edit raw</Kbd> → Save — ไฟล์จริงจะถูกลบจาก disk ให้อัตโนมัติ</li>
