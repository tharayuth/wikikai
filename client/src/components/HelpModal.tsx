    ...
    <div className="help-content">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td>Caption a picture or a block</td><td>A diagram, chart or card carries its caption in the <code>{'{'}@N "…"{'}'}</code> annotation; a picture carries it in the title slot — <code>![alt](src "Overall system w=520")</code>. Either way it renders as small italic text centred under the block. A picture only gets the visible caption when it stands alone in its paragraph; one inside a sentence keeps the text as a tooltip so the prose is not cut in half</td></tr>
          <tr><td>Resize a diagram</td><td>Hover a Mermaid diagram and drag the handle on its <strong>bottom edge</strong>; the diagram scales to the height you pick and its width follows, so the drawing grows or shrinks as a whole. This also lifts the default height cap that otherwise crops a tall diagram. Double-click the handle to go back to the natural size. Unlike the reading column, this is saved into the document — everyone who opens it, including via a share link, sees the size you chose</td></tr>
    ...
    <div className="help-content">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td>ใส่ caption ให้ภาพหรือ block</td><td>diagram / chart / card เก็บ caption ไว้ใน annotation <code>{'{'}@N "…"{'}'}</code> ส่วนภาพเก็บไว้ใน title slot — <code>![alt](src "แผนผังระบบ w=520")</code>. ทั้งสองแบบแสดงเป็นตัวเอียงเล็กจัดกึ่งกลางใต้ block เหมือนกัน. ภาพจะขึ้น caption ก็ต่อเมื่อ <strong>อยู่ลำพังในย่อหน้าของตัวเอง</strong> ส่วนภาพที่แทรกกลางประโยคจะเก็บข้อความไว้เป็น tooltip แทน เพื่อไม่ให้ตัดเนื้อความขาดกลางย่อหน้า</td></tr>
          <tr><td>ย่อ/ขยาย diagram</td><td>เอาเมาส์ชี้ที่ diagram (Mermaid) แล้วลากที่ <strong>ขอบล่าง</strong> — diagram จะขยาย/ย่อตามความสูงที่เลือก ส่วนความกว้างไปตามสัดส่วนเอง ภาพจึงใหญ่/เล็กทั้งอัน. วิธีนี้ยังปลดเพดานความสูงเริ่มต้นที่ปกติจะตัด diagram สูง ๆ ทิ้งด้วย. ดับเบิลคลิกที่ขอบเพื่อคืนขนาดปกติ. ต่างจากความกว้างคอลัมน์อ่านตรงที่ค่านี้ <strong>บันทึกลงในเอกสาร</strong> — ทุกคนที่เปิด รวมถึงคนที่เปิดผ่านลิงก์แชร์ จะเห็นขนาดเดียวกับที่คุณตั้ง</td></tr>
