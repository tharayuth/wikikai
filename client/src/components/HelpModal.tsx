  ...
  return (
    <div className="help-content">
      ...
      <ul>
        ...
        <li><strong>Interactive checkboxes</strong> — write a GFM task list <code>- [ ] item</code> / <code>- [x] item</code> anywhere a markdown list goes; the renderer turns each into a real clickable checkbox. <em>Clicking a box writes back to the source immediately</em> (page version bumped, revision snapshot, FTS reindexed). AI flips them via the <code>toggle_task</code> MCP tool — e.g. "tick task 3 on this page". Also works inside <code>html-embed</code> tables/cards as visual <code>&lt;input type="checkbox"&gt;</code> markup (display-only). The legacy <code>```checklist</code> JSON fence is kept for back-compat (paired with <code>toggle_checklist_item</code>) for documents that need a titled progress card with an <code>@N</code> id</li>
  ...
  return (
    <div className="help-content">
      ...
      <ul>
        ...
        <li><strong>Interactive checkbox</strong> — เขียน GFM task list <code>- [ ] item</code> / <code>- [x] item</code> ใน list ปกติ; renderer จะแปลงเป็น checkbox จริงคลิกได้. <em>คลิกแล้ว save กลับ markdown ทันที</em> (bump page version + revision snapshot + FTS reindex). AI ใช้ MCP tool <code>toggle_task</code> ก็ได้ผลเดียวกัน (เช่น "tick task 3"). ใช้ใน <code>html-embed</code> table/card ก็ได้แต่ <code>&lt;input type="checkbox"&gt;</code> แบบนั้นเป็น display-only. รูปแบบเก่า <code>```checklist</code> JSON fence ยังใช้ได้ (คู่กับ <code>toggle_checklist_item</code>) สำหรับการ์ด titled มี progress bar + <code>@N</code> id</li>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>toggle_task</code></td><td>Flip a plain <code>- [ ]</code> / <code>- [x]</code> task on a page. Args: <code>{`{ page_id, index }`}</code> where <code>index</code> is the 0-based position of the checkbox top-down (skipping any inside fenced code). Same write-back path the rendered UI uses. <strong>Preferred form</strong> for new docs</td></tr>
          <tr><td><code>toggle_checklist_item</code></td><td>Legacy: flip the <code>done</code> state of one item in the JSON <code>```checklist</code> fence. Args: <code>{`{ block_id, index, done }`}</code>. Use only when the doc carries the older titled progress-card form</td></tr>
        </tbody>
      </table>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>toggle_task</code></td><td>กลับสถานะ <code>- [ ]</code> / <code>- [x]</code> ที่หน้านั้น. Args: <code>{`{ page_id, index }`}</code> โดย <code>index</code> เป็นเลข 0-based นับจากบนลงล่าง (ข้าม task ใน fenced code). เส้นทางเดียวกับ web UI ตอนคลิก checkbox. <strong>รูปแบบที่แนะนำ</strong> สำหรับเอกสารใหม่</td></tr>
          <tr><td><code>toggle_checklist_item</code></td><td>Legacy: กลับสถานะ <code>done</code> ของ item ใน JSON <code>```checklist</code> fence. Args: <code>{`{ block_id, index, done }`}</code>. ใช้เฉพาะเมื่อเอกสารใช้รูปแบบ titled progress-card เก่า</td></tr>
        </tbody>
      </table>
