    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>add_file</code></td><td>Attach a downloadable file — send <code>{`{ data_base64, name }`}</code> or <code>{`{ path }`}</code> (same import roots as add_image) plus an optional <code>description</code>. Returns <code>{`{ src, name, size_bytes, mime, url, fence }`}</code>; paste <code>fence</code> into a page as a <code>```file</code> block. Bytes are deleted automatically once no page references them</td></tr>
    ...
    "wikikai": {
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>add_file</code></td><td>แนบไฟล์ให้ดาวน์โหลด — ส่ง <code>{`{ data_base64, name }`}</code> หรือ <code>{`{ path }`}</code> (root เดียวกับ add_image) + <code>description?</code>. คืน <code>{`{ src, name, size_bytes, mime, url, fence }`}</code> — วาง <code>fence</code> ลงหน้าได้เลย เป็น <code>```file</code> block. ไฟล์จริงถูกลบเองเมื่อไม่มีหน้าไหนอ้างถึงแล้ว</td></tr>
