  ...
  server.registerTool(
    "add_image",
    {
      ...
      description:
        ...
        "  • Plain markdown `![alt](<src>)` — the most flexible. Works in paragraphs, list items, AND markdown table cells. Use when the image is inline content (a screenshot in prose, a thumbnail inside a comparison table). Example table cell: `| Logo | ![brand](/img/abc….png) | … |`. Optional size via the title slot — `![alt](<src> \"WxH\")` (e.g. `\"300x200\"` fits both, `\"300x\"` width-only, `\"x200\"` height-only, or `\"caption w=300 h=200\"` to mix with caption text); aspect ratio is always preserved (uses max-width / max-height + auto on the other axis). **Keep inline sizes modest** — the article column is ~860px wide on a typical screen and narrower on small viewports; a `\"1440x\"` hint will be clamped to the column width by CSS but still wastes bytes. For larger reference / screenshot images that should expand on click, use the ```images fence below instead.\\n" +
        "  • ```images fence — JSON array of `{ src, alt?, caption? }`; renders a thumbnail grid with click-to-lightbox. **Preferred for any large screenshot / showcase image** — readers see a small preview and can click to view the original at full resolution, so the source can be any size.\\n" +
