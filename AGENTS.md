...
This file is the project orientation for every coding agent — Codex reads it
directly, and Claude Code reads it through the one-line `@AGENTS.md` import
in `CLAUDE.md` (so machine-specific `CLAUDE.local.md` keeps working). Edit
this file, never `CLAUDE.md`. It is what an experienced contributor would
tell a new collaborator before they touch any code.
...
Machine-specific context goes in `CLAUDE.local.md`, which is gitignored
(Claude Code reads it next to this file; Codex does not). Ops
