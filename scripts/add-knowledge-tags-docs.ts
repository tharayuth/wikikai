...
\`\`\`steps
[
  { "title": "เปิดข้อมูล knowledge", "body": "เปิด topic ที่ต้องการ แล้วคลิกปุ่ม **i** ข้างชื่อ knowledge" },
  { "title": "เปิด tag editor", "body": "ที่แถว **tags** คลิก **+ Add tags** หรือ **Edit**" },
...
if (existingTagPage) {
  console.log(`&4 #${existingTagPage.id}: knowledge-tags tutorial already exists`);
} else {
...
// ─── &3 #18: explain the Thailand-themed tag set ───
{
  ...
  if (!page) {
    ...
    console.log("&3 #18: knowledge-tags showcase already exists");
  } else {
    const section = `
${marker}
